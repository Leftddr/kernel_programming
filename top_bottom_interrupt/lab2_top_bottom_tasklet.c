...
#include "lab_top_bottom_interrupt.h"
#define MYIOC_TYPE 'k'
...
static void t_fun(unsigned long t_arg)
{
	...
	*/
	if(START_BOT == 1){
		if(botprevjiffies == -1) botprevjiffies = jiffies;
		else {
			unsigned long interval = jiffies - botprevjiffies;
			if(interval < data.bminjiffies) data.bminjiffies = interval;
			if(interval > data.bmaxjiffies) data.bmaxjiffies = interval;
			botforjiffies += interval;
		}
		botN++;
		pr_info("BOTTOM-HALFS : N = %u, MAX_jiffies = %ld, MIN_jiffies = %ld, Aver_jiffies = %ld\n", botN, data.bmaxjiffies, data.bminjiffies, (botforjiffies / botN));
		if(botN == 1000){
			START = 0; START_BOT = 0; botN = 0; botprevjiffies = -1;
		}
	}
}
...
static irqreturn_t my_interrupt(int irq, void *dev_id)
{
	...
	if(START_TOP == 1){
		if(topprevjiffies == -1) topprevjiffies = jiffies;
		else {
			unsigned long interval = jiffies - topprevjiffies;
			if(interval < data.tminjiffies) data.tminjiffies = interval;
			if(interval > data.tmaxjiffies) data.tmaxjiffies = interval;
			topforjiffies += interval;
		}
		topN++;
		pr_info("TOP-HALFS : N = %u, MAX_jiffies = %ld, MIN_jiffies = %ld, Aver_jiffies = %ld\n", topN, data.tmaxjiffies, data.tminjiffies, (topforjiffies / topN));
		if(topN == 1000){
			START_TOP = 0; topN = 0; topprevjiffies = -1;
		}
	}
...
mycdrv_unlocked_ioctl(struct file *fp, unsigned int cmd, unsigned long arg)
{
	...
	case _IOC_READ:
		if(size <= 8)
			...
			rc = copy_to_user(ioargp, &data, size);
		
		return rc;
		break;
	case _IOC_WRITE:
		rc = copy_from_user(&START, ioargp, size);
		data.bminjiffies = 1e12; data.bmaxjiffies = 0; botforjiffies = 0;
		data.tminjiffies = 1e12; data.tmaxjiffies = 0; topforjiffies = 0;
		pr_info("START = %u\n", START);
		START_TOP = START;
		START_BOT = START;
		return rc;
