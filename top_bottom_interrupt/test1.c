...
struct my_data{
	unsigned long int max_jiffies, min_jiffies, aver_jiffies;
}my_data;
int START = 1;
#define MYIOC_TYPE 'k'
...
int main(int argc, char *argv[])
{
	...
	if(ch == 'Y'){
		...
		printf("max_jiffies = %ld, min_jiffies = %ld, aver_jiffies = %ld\n", my_data.max_jiffies, my_data.min_jiffies, my_data.aver_jiffies); 
